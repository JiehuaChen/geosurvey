...
if(dim(cov_neighbors_withloc)[1]>0){
    ...
    kernel_predicts <- extract(stack("/home/ubuntu/kernel_maps/CRPKDs.tif"), locs) 
    ...
    glm_ens <- h2o.loadModel(localH2O, "/home/ubuntu/h2omodels/glm_ens_crp_withkernel")
    ens_coef <- glm_ens@models[[glm_ens@best_model]]@model$coefficients

    rf_model <- h2o.loadModel(localH2O, "/home/ubuntu/h2omodels/rf_model")
    gbm_model <- h2o.loadModel(localH2O, "/home/ubuntu/h2omodels/gbm_model")
    nnet_model <- h2o.loadModel(localH2O, "/home/ubuntu/h2omodels/nnet_model")
	...
	while (tiffile_upload != 0){
        tiffile_upload <- system(paste("s3cmd put -f", predict_file, "s3://afsis.geosurvey/results_h2o_crp_withkernel/"))
    }
